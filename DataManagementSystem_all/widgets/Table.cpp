...
Table::Table(int row, int col) 
	...
	,BasicWidget(0,0,0,0)
{
	m_prevPageBtn.setText("上一页"); //上一页按钮
	m_nextPageBtn.setText("下一页"); //下一页按钮
	m_beginPageBtn.setText("第一页");//第一页按钮
	m_endPageBtn.setText("结尾页");  //结尾页按钮
}
...
void Table::setHeader(const std::string& data)//表头设置
{
	m_header = data;//表头为字符串
	m_cols = std::count(m_header.begin(), m_header.end(), '\t') - 1;//数这字符串出现几个'\t'，算出表格一共有几列

	int textw = ::textwidth("最佳拟合阶数");//文字的宽度 最宽文字的宽度
	int texth = ::textheight(m_header.c_str());//文字的高度
...
void Table::eventLoop(const ExMessage& msg)
{
	...
	m_endPageBtn.eventLoop(msg);//四个按钮

	if (m_prevPageBtn.isClicked())//上一页
	{
		if (m_curPage != 0)
		{
			m_curPage--;
	...
	if (m_nextPageBtn.isClicked())//下一页
	{
	...
	if (m_beginPageBtn.isClicked())//首页
	{
		m_curPage = 0;
	}
	if (m_endPageBtn.isClicked())//尾页
	{
		m_curPage = m_maxPage;
	}
}
...
void Table::drawHeader()
{
	...
	setlinecolor(BLACK);
	setlinestyle(PS_SOLID, 1);
	rectangle(m_x, m_y - 40, m_x + m_gridw * m_cols, m_y);
	for (size_t i = 0; i < m_cols + 1; i++)
	{
		line(m_x + i * m_gridw, m_y - 40, m_x + i * m_gridw, m_y);
	}
	//绘制表头数据
